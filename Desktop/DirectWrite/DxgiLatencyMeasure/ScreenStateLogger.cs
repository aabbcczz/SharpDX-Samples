...
namespace DxgiLatencyMeasure
{
    internal sealed class ScreenStateLogger
    {
        ...
        {
            var rects = new Rectangle[] { new Rectangle(0, 0, 0, 0) };

            Run(rects, screenRefreshFunction);
        }

        public bool Run(IntPtr hwnd, Rectangle[] clientAreas, Func<Bitmap[], long, bool> screenRefreshFunction)
        {
        ...
        public void Run(Rectangle[] screenAreas, Func<Bitmap[], long, bool> screenRefreshFunction)
        {
                    ...
                    var callbackTask = Task.Run(() =>
                    {
                        while (!stopped)
                        {
                            ...
                            for (int i = 0; i < workItem.resources.Count; ++i)
                            {
                                ...
                                var rect = new Rectangle(0, 0, screenAreas[i].Width, screenAreas[i].Height);
